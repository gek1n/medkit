...
class BillingLifecycleService {
  ...
  ///
  /// Для тестування повного флоу (поп-ап → очікування → авторозрив) на
  /// реальних пристроях, не чекаючи реальні 5 днів — стан підписки міняється
  /// напряму в БД (`subscriptions.status`) через phpMyAdmin, окремого
  /// debug-UI не потрібно, `/subscription/status` і так читає прямо з
  /// таблиці (DEPLOY.md, крок 13.0); для швидшого локального прогону можна
  /// тимчасово зменшити цю константу в дебагері, але комітити в git — лише
  /// зі справжніми 5 днями (білінг уже реальний, живі платні користувачі).
  static const gracePeriod = Duration(days: 5);
