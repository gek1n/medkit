...
class Members extends Table {
  ...
  // Порядок відображення у перемикачах "хто зараз активний" (Сьогодні,
  // MemberSwitcherPill на Розкладі/Медкартці) — керується драг-н-дропом на
  // екрані Сім'я. Власник НІКОЛИ не входить у драг-н-дроп (завжди перший,
  // поза списком) і лишається на дефолтних 0 — реордер серед залежних
  // профілів навмисно рахує з 1, щоб жоден із них не міг випадково
  // "обігнати" власника значенням 0.

  TextColumn get linkedPeerPersonUuid => text().nullable()();
  // Крок 7.1 плану: заповнено лише для "тіньового" dependent-рядка, який
  // існує НЕ як реальний профіль, а як локальний "слот", що представляє
  // автономного члена сім'ї (FamilyPeers.personUuid) у пулі ротації
  // рутинної справи (ActivityAssignees.memberId) — без цього поле
  // memberId довелось би робити полiморфним (nullable + окреме посилання
  // на пір) у ActivityAssignees/ActivityLogs і всюди, де вони читаються
  // (Сьогодні, сповіщення, family_duties_screen тощо). Свідомо НЕ
  // personUuid (те поле вже має власне значення — стабільна ідентичність
  // САМОГО цього рядка; тут навпаки, рядок лише посилається на ЧУЖУ
  // ідентичність). Автономний пір впізнає "це я" звірянням
  // linkedPeerPersonUuid зі своїм ВЛАСНИМ Members(role=owner).personUuid.
}
