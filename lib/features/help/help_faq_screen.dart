...
const _groups = <_FaqGroup>[
  _FaqGroup('Приватність і дані', Icons.lock_outline_rounded, [
    _Faq(
      ...
      'Ніхто, крім вас. Усе зберігається зашифрованим на вашому пристрої '
          '(SQLCipher, AES-256). Сервер MedKit навмисно "сліпий" — акаунтів '
          'немає, і те, що через нього проходить (пейринг, синхронізація), '
          'бачить лише зашифровані блоки без ключа.',
    ),
    _Faq(
      'У чому різниця між Синхронізацією, Резервною копією і Пейрингом?',
      'Резервна копія — знімок ваших даних у власному Google Drive/iCloud '
          'на випадок втрати телефону. Синхронізація — опційний живий '
          'зв\'язок із власним сервером MedKit (той самий акаунт на новому '
          'пристрої). Пейринг — обмін розкладом між РІЗНИМИ людьми в родині '
          '(наприклад, дитина бачить розклад мами). Це три різні механізми, '
          'не дублікати одне одного.',
    ),
    _Faq(
  ...
  _FaqGroup('Сім\'я', Icons.family_restroom_rounded, [
    ...
    _Faq(
      'Як підключити другий телефон до профілю?',
      'На картці члена сім\'ї — "Підключити телефон": згенеруйте код або '
          'QR на одному пристрої, введіть на іншому. Дані шифруються цим '
          'кодом, сервер бачить лише зашифрований блок.',
    ),
    _Faq(
  ...
  _FaqGroup('Сповіщення', Icons.notifications_none_rounded, [
    _Faq(
      ...
      'Найчастіша причина — оптимізація батареї на Android обмежує '
          'фонову роботу застосунку. Додайте MedKit у виключення в '
  ...
  _FaqGroup('Тарифи', Icons.workspace_premium_outlined, [
    _Faq(
      ...
      'Безкоштовний — базові функції з обмеженнями. Плюс і Сім\'я знімають '
          'ліміти й додають розширені можливості. Деталі — Профіль → Тарифи.',
    ),
  ]),
  _FaqGroup('Технічні проблеми', Icons.build_outlined, [
    _Faq(
      'Не працює біометрія / забув пароль від резервної копії',
      'Пароль резервної копії ніде не зберігається, крім як у вашій '
          'пам\'яті, — без нього відновити копію неможливо, доведеться '
          'створити нову. Біометрію можна переналаштувати в системних '
          'налаштуваннях пристрою.',
    ),
    _Faq(
      'Apple Sign-In не працює на Android',
      'Відоме обмеження: на Android Apple Sign-In поки повністю не '
          'налаштований, працює лише на iOS/macOS. Використовуйте Google '
          'або recovery key.',
    ),
  ]),
];
      ...
      child: Column(
        ...
        children: [
          ...
          _ContactRow(
            ...
            label: 'Питання щодо конфіденційності',
            value: 'privacy@elly-medkit.com',
            onTap: () => _openMail('privacy@elly-medkit.com'),
          ),
          const SizedBox(height: AppDimensions.sm),
          _ContactRow(
