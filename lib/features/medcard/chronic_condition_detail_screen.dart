  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            MkFormHeader(title: 'Хронічне захворювання', onBack: () => Navigator.pop(context)),
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  if (condition.specialty?.isNotEmpty ?? false)
                    MkDetailRow(label: 'Напрямок', value: Text(condition.specialty!, style: AppTextStyles.bodyMd)),
                  if (condition.diagnosedAt != null)
                    MkDetailRow(
                      label: 'Дата діагнозу',
                      value: Text(MKDateUtils.formatDate(condition.diagnosedAt!), style: AppTextStyles.bodyMd),
                    ),
                  if (condition.notes?.isNotEmpty ?? false)
                    MkDetailRow(label: 'Нотатки', value: Text(condition.notes!, style: AppTextStyles.bodyMd)),
