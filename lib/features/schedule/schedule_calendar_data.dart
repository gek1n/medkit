...
// перекладеного кешу даних піра (Крок 4.3), а не з локальної БД — генератори
// (intake/activityLog) тут не потрібні: пір присилає вже згенеровані
// intake/activity_log записи власним синком, повторно генерувати їх тут
// нема сенсу (і нема звідки — локальних Medications/Activities піра не
// існує). Реальний баг: раніше Календарний вигляд для піра просто не
// показувався (schedule_screen.dart мовчки падав назад на список) — окрема
// задача, свідомо відкладена до появи повного перекладача (Крок 4.3).
